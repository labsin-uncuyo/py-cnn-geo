...
def main(argv):
    ...
    print('Preparing for SVM parameter search')
    dataset_folder = None
    neighbors = None
    ...
    try:
        ...
        print('svm_search.py -s <dataset_folder>')
        sys.exit(2)
    for opt, arg in opts:
        if opt == "-h":
            print('svm_search.py -s <dataset_folder>')
    ...
    store_sub_path = '../storage/svm-search/cir/'
...
def parameter_searcher(dataset_folder, neighbors, feature_reduction, use_vector_features, search_status, store_sub_path,
    ...
    name_args = ['alpha', 'loss', 'penalty']
    param_grid = dict(alpha=alpha, loss=loss, penalty=penalty, seed=[seed])
