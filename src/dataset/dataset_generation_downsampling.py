...
def main(argv):
    ...
    try:
        ...
        print('dataset_generation_downsampling.py -s <source_folder>')
        sys.exit(2)
    for opt, arg in opts:
        if opt == "-h":
            print('dataset_generation_downsampling.py -s <source_folder>')
...
def explore_dataset_folders(src_folder):
    ...
    #bigdata = np.zeros(shape=(len(sample_rasters_folders), 12, 3601, 3601), dtype=np.float32)
    #bigdata_gt = np.zeros(shape=(len(sample_rasters_folders), 3600, 3600), dtype=np.uint8)

    #pckmetadata = np.empty(shape=(len(sample_rasters_folders), 12), dtype=object)
    #pckmetadata_gt = np.empty(shape=(len(sample_rasters_folders), 7), dtype=object)
    ...
    for i, pck in enumerate(sample_rasters_folders):
        ...
        spl_selector = SampleSelectorThread(i, path_to_pck, redis_db)
        sample_selectors.append(spl_selector)
